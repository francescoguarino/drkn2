...
export class MinimalNetworkManager extends EventEmitter {
    ...
    setupHandlers() {
        ...
        // Message handling
        this.node.handle('/minimal/1.0.0', async ({ stream }) => {
            try {
                const { value } = await stream.source.next();
                // Usa la libreria corretta per la conversione
                const message = JSON.parse(uint8ArrayToString(value));
    ...
    async connectTobootstrapNodes() {
        for (const node of this.config.bootstrapNodes) {
            try {
                const ma = multiaddr(node);
                await this.node.dial(ma);
                this.logger.info(`Connesso a ${node}`);
            } catch (error) {
                this.logger.error(`Errore durante la connessione a ${node}:`, error);
            }
        }
    }
    ...
    async start() {
        try {
            ...
            // Aggiungi 'let' per dichiarare la variabile
            ...
            await this.sendMessage( multiaddr('/ip4/34.147.53.15/tcp/6001/p2p/12D3KooWFYbYCGEsYQY71sCfUJFAzBtDJKuZjHonqxbjndPW5Jje'), {
                type: 'hello',
                message: 'Ciao dal nodo ' + this.node.peerId.toString()
            });
            this.logger.info('Messaggio di saluto inviato al nodo bootstrap');
