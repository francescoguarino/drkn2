...
if (process.argv[1].endsWith('node-runner.js')) {
  const options = parseCommandLineArgs();
  runNode(options)
    .then(() => {
      console.log('NODO IN ESECUZIONE .............');
    })
    .catch(err => {
...
Esempi:
  node src/node-runner.js --port 6001
  node src/node-runner.js --port 6001 --data-dir ./node-data
  `);
}
